...
import { register, createDealership } from "@/actions/register";
...
type RegisterDealerSchemaType = z.infer<typeof RegisterDealerSchema>;
...
export const RegisterDealerForm = () => {
  ...
  const form = useForm<RegisterDealerSchemaType>({
  ...
  const handleFileUpload = async (file: File) => {
    ...
    const response = await fetch("/api/upload-pdf?type=dealership", {
      method: "POST",
      body: formData,
    });
  ...
  const onSubmit = async (values: RegisterDealerSchemaType) => {
    try {
      setIsPending(true);
      setError("");
      setSuccess("");
      ...
      // Step 1: Register the user as a dealer
      const registerResponse = await register(registerValues, "DEALER");
      ...
      // Step 3: Create the dealership record
      if (registerResponse?.userId) {
        const dealershipResponse = await createDealership(
          registerResponse.userId,
          registerValues.dealershipName,
          registerValues.phoneNumber,
          pdfFileUrl
        );

        if ("error" in dealershipResponse) {
          setError(dealershipResponse.error);
        } else {
          setSuccess(dealershipResponse.success);
        }
      }
  ...
  return (
    <CardWrapper
      headerLabel="Create a Dealer Account"
          ...
          <div className="space-y-4">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      {...field}
                      disabled={isPending}
                      placeholder="Best Cars Dealership"
                      type="text"
                    />
                  </FormControl>
                  <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Upload Dealership Certificate (PDF only, max 4MB)
                  </FormLabel>
                  <FormControl>
                    <FileInput
                      label=""
